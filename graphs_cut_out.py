...
def plot_results(**kwargs):
    if os.path.isfile('results/cut_out/FSM_cut_out.csv'):
        ...
        dfHMM = pd.read_csv('results/cut_out/HMM_cut_out.csv')
    else:
    ...
    for ur in range(10, 131, 10):
        plt.figure(str(ur) + ' HMM')
        plt.title('HMM')
        df = dfHMM[dfHMM['velocity'] == ur]

        HMM_mask1 = (df['Crash']) & (df['Crash_front'] != True)
        HMM_mask2 = (~df['Crash']) & (~df['Crash_front'])
        HMM_mask3 = df['Crash_front']

        plt.plot(df[HMM_mask1]['front_distance'], -df[HMM_mask1]['lateral_velocities'], 'rx', markersize=10, alpha=0.2,
                 label='Crash')
        plt.plot(df[HMM_mask2]['front_distance'], -df[HMM_mask2]['lateral_velocities'], 'go', markersize=10, alpha=0.2,
                 label='Safe')
        plt.plot(df[HMM_mask3]['front_distance'], -df[HMM_mask3]['lateral_velocities'], 'yo', markersize=10, alpha=0.2,
                 label='Crash leader')
        ...
        plt.tight_layout()
        if kwargs.get('save_image', False):
            plt.savefig('results/cut_out/HMM_' + str(ur) + '.png')
            plt.close()
        else:
            plt.show()
